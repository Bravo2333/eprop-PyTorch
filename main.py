...
------------------------------------------------------------------------------
 ...
 "main.py" - Main file for training a spiking RNN with eligibility propagation (e-prop), as per the original paper:
    ...
    Example run: default parameters contained in this file provide suitable convergence for the cue accumulation task
             described in the e-prop paper. The code can just be run with "python main.py"
...
def main():
    ...
    parser.add_argument('--lr-layer-norm', type=float, nargs='+', default=(0.05,0.05,0.05), help='Per-layer modulation factor of the learning rate')
    parser.add_argument('--batch-size', type=int, default=50, help='Batch size for training (limited by the available GPU memory)')
    parser.add_argument('--test-batch-size', type=int, default=50, help='Batch size for testing (limited by the available GPU memory)')
    ...
    parser.add_argument('--n-rec', type=int, default=168, help='Number of recurrent units')
    ...
    parser.add_argument('--tau-mem', type=float, default=1500e-3, help='Membrane potential leakage time constant in the recurrent layer (in seconds)')
    ...
    (device, train_loader, traintest_loader, test_loader) = datasolver.setup(args)
